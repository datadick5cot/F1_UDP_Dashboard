...
# def udp_listener(UDP_IP, UDP_PORT, history_lock, player_history,rival_history):
#     """
#     Continuously read UDP packets, track player & rival telemetry,
#     and keep data limited to the current lap only.
#     """
#     global current_lap_number, latest_player_lap, latest_rival_lap
...
#     # rival_car_index = 21  # Adjust as needed for rival
#     TotalLap = 0
...
#         if packet_id == 0:  # Motion packet
#             motion_packet = PacketMotionData.from_buffer_copy(data)
#             car_motion = motion_packet.m_carMotionData[player_car_index]

#             # RL, RR, FL, FR
#             WheelSpeed = list(car_motion.m_wheelSpeed)
#             WheelSplipRatio = list(car_motion.m_wheelSlipRatio)
#             WheelSplipAngle = list(car_motion.m_wheelSlipAngle)
...
#                     "WheelSplipRatio" : WheelSplipRatio,
#                     "WheelSplipAngle" : WheelSplipAngle,
#                     "WheelLat" : WheelLat, 
#                     "WheelLong" : WheelLong
#                 })
...
#             TotalLap = packet.m_trackLength

#         # --- LAP DATA (detect lap changes and update cache) ---
#         elif packet_id == 2:  # LAP_DATA
#             rival_car_index = packet.m_timeTrialRivalCarIdx
            
#             # print(f'rival at index: {rival_car_index}')
#             latest_player_lap = packet.m_lapData[player_car_index]
#             latest_rival_lap = packet.m_lapData[rival_car_index]

#             lap_number = latest_player_lap.m_currentLapNum

#             # Detect new lap and clear histories
#             if current_lap_number is None:
#                 current_lap_number = lap_number

#             if lap_number != current_lap_number:
#                 print(f"[LAP] New lap detected: {lap_number}. Clearing histories...")
#                 with history_lock:
#                     player_history.clear()
#                     rival_history.clear()
#                 current_lap_number = lap_number

#         elif packet_id == 6 and latest_player_lap and latest_rival_lap:
#             # Player telemetry
#             player_data = packet.m_carTelemetryData[player_car_index]
#             rival_data = packet.m_carTelemetryData[rival_car_index]

#             # Extract values
#             player_throttle = float(player_data.m_throttle)
#             player_brake = float(player_data.m_brake)
#             player_steer = float(player_data.m_steer)
#             player_speed = int(player_data.m_speed)
#             player_accel_grip = player_throttle - player_brake
#             player_gear = int(player_data.m_gear)

#             rival_throttle = float(rival_data.m_throttle)
#             rival_brake = float(rival_data.m_brake)
#             rival_speed = int(rival_data.m_speed)
#             rival_gear = int(rival_data.m_gear)

#             # Get lap distance info safely
#             PlayerLap = latest_player_lap.m_lapDistance
#             RivalLap = latest_rival_lap.m_lapDistance

#             # Append both to their histories
#             with history_lock:
#                 player_history.append({
#                     "acceleration_grip": player_accel_grip,
#                     "throttle": player_throttle,
#                     "brake": player_brake,
#                     "steer": player_steer,
#                     "speed": player_speed,
#                     "gear": player_gear,
#                     "TotalDistance": TotalLap,
#                     "DistanceLap": PlayerLap,
#                 })
#                 rival_history.append({
#                     "throttle": rival_throttle,
#                     "brake": rival_brake,
#                     "speed": rival_speed,
#                     "gear": rival_gear,
#                     "DistanceLap": RivalLap,
#                 })
...
#     target=udp_listener,
#     args=("0.0.0.0", 20777, history_lock, player_history, player_motion, rival_history),
#     daemon=True
# )
# listener_thread.start()

# print("UDP listener running... waiting for telemetry data...")
...
#             print(f"Latest Player Data: ratio={latest['WheelSplipRatio']}Speed={latest['speed']}, Throttle={latest['throttle']:.2f}, Brake={latest['brake']:.2f}")
#         else:
...
def udp_listener(UDP_IP="0.0.0.0", UDP_PORT=20777):
    """
    Listen for all F1 24 UDP packets and route to correct storage.
    """
    global current_lap_number, latest_player_lap, latest_rival_lap
    ...
    while True:
        ...
        with history_lock:
            ...
            # --- 2: Lap Data ---
                ...
                latest_player_lap = packet.m_lapData[player_car_index]
                rival_idx = packet.m_timeTrialRivalCarIdx
                latest_rival_lap = packet.m_lapData[rival_idx]

                lap_data_history.append({
                    "player": latest_player_lap.m_currentLapNum,
                    "distance": latest_player_lap.m_lapDistance,
                ...
                if latest_player_lap.m_currentLapNum != current_lap_number:
                    print(f"[LAP] New lap {latest_player_lap.m_currentLapNum} — resetting histories")
            ...
            # --- 5: Car Setups ---
                ...
                car_setups_data.clear()
                car_setups_data.append({"frontWing": packet.m_carSetups[player_car_index].m_frontWing})

            # --- 6: Car Telemetry ---
            elif packet_id == 6:
                car_data = packet.m_carTelemetryData[player_car_index]
                car_telemetry_history.append({
                    "speed": car_data.m_speed,
                    "throttle": car_data.m_throttle,
                    "brake": car_data.m_brake,
                    "steer": car_data.m_steer,
            ...
            # --- 8: Final Classification ---
                ...
                classification_data.append({"positions": [c.m_position for c in packet.m_classificationData]})

            # --- 9: Lobby Info ---
            elif packet_id == 9:
                lobby_info_data.clear()
                lobby_info_data.append({"players": packet.m_numPlayers})

            # --- 10: Car Damage ---
            elif packet_id == 10:
                damage = packet.m_carDamageData[player_car_index]
                car_damage_history.append({
                    "tyreWear": list(damage.m_tyresWear),
                    "wingDamage": list(damage.m_wingDamage)
                })
            ...
            # --- 11: Session History ---
                ...
                session_history_data.append({
                    ...
                    "bestLapTime": packet.m_bestLapTimeLapNum
                })
            ...
            # --- 12: Tyre Sets ---
                ...
                tyre_sets_data.append({
                    "numTyreSets": packet.m_numTyreSets,
                    "available": [t.m_tyreLife for t in packet.m_tyreSets]
                })
            ...
            # --- 14: Time Trial ---
                ...
                time_trial_data.append({
                    "rivalIndex": packet.m_rivalCarIndex,
                    "bestLapTime": packet.m_bestLapTime
                })


# --- Start listener thread for testing ---
